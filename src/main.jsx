...
import { createMicStreamer as makeMicStreamer } from './mic-streamer.js'
...
const api = async (url, options = {}) => {
  const response = await request(url, { ...options, headers: { 'Content-Type': 'application/json', ...adminHeaders(), ...(options.headers || {}) } })
...
function createMicStreamer(options = {}) {
  return makeMicStreamer({ headers: adminHeaders, onExpired: dropExpiredAdmin, ...options })
}
function trustedAdminPage() {
  return location.protocol === 'https:' || ['localhost', '127.0.0.1', '[::1]', '::1'].includes(location.hostname)
}
function secureAdminUrl(station) {
  const url = new URL(location.href)
  url.protocol = 'https:'
  url.port = String(station.network.httpsPort || station.network.https?.port || 8443)
  url.pathname = '/listen'; url.search = ''; url.hash = 'admin'
  return url.href
}
function openSecureAdmin(station) {
  if (station.network.https && !station.network.https.listening) {
    alert(station.network.https.error || 'Güvenli yönetim sayfası hazırlanıyor. Birkaç saniye sonra tekrar deneyin.')
    return false
  }
  location.assign(secureAdminUrl(station))
  return true
}
...
async function adminLogin(code) {
  if (!trustedAdminPage()) throw new Error('Yönetici girişi için HTTPS sayfasını açın.')
  const response = await request('/api/admin/login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ code }) })
...
function adminLogout() {
  ...
  try { localStorage.removeItem('cr_admin_token') } catch {}
  if (token && trustedAdminPage()) request('/api/admin/logout', { method: 'POST', headers: { 'x-admin-token': token } }).catch(() => {})
}
...
function useStation() {
  ...
  useEffect(() => {
    let events, retry, closed = false, generation = 0
    const connect = () => {
      const attempt = ++generation
      let receivedEvent = false
      const current = () => !closed && attempt === generation
      api('/api/state').then(snapshot => { if (current() && !receivedEvent) setStation(snapshot) }).catch(() => {})
  ...
  const upload = async files => {
    for (const file of files) {
      ...
      try {
        ...
        const res = await request('/api/media/' + kind, { method: 'POST', headers: { ...adminHeaders() }, body }, 30000)
...
const ICONS = {
  ...
  loop: svgIcon(<path d="M17 2l4 4-4 4V7H8a4 4 0 0 0 0 8h1v2H8A6 6 0 0 1 8 5h9V2zm-1 9h1a6 6 0 0 1 0 12H8v3l-4-4 4-4v3h9a4 4 0 0 0 0-8h-1v-2z"/>),
  ...
  return <section className="player">
    ...
    <div className="player-main">
      <div className="player-buttons">
        ...
        <button className={'pbtn' + (pb.loop !== false ? ' on' : '')} title={pb.loop !== false ? 'Tekrar açık' : 'Tekrar kapalı'} onClick={() => save({ playback: { loop: pb.loop === false } })}>{ICONS.loop}</button>
    ...
    {!status.supported
      ...
      : status.downloaded
        ...
        : status.downloading
          ...
          : <>
              <p className="muted">{status.checking ? 'Kontrol ediliyor…' : status.noReleaseYet ? 'Henüz yayımlanmış bir sürüm yok.' : 'Program güncel.'}</p>
...
function ConnectCard({ station }) {
  ...
  return <section className="card connect-card">
    ...
    {net.https?.error && <p role="status" className="bad">{net.https.error} Yönetici/anons bağlantısı için HTTPS durumu düzelene kadar bekleyin.</p>}
...
function Admin() {
  ...
  const [localVol, setLocalVol] = useState(() => readStoredVolume('cr_local_vol'))
  useEffect(() => { if (audio.current) audio.current.volume = Math.max(0, Math.min(1, localVol / 100)) }, [localVol, Boolean(station)])
  ...
  const control = async (action, value) => {
    const updated = await api('/api/control', { method: 'POST', body: JSON.stringify({ action, value }) })
    // A track picked from the library (or a skip/manual ad) starts the station on the
    // server just like Play does.  Keep the desk monitor's intent in step with those
    // user initiated transport actions; otherwise the first selected song is audible on
    // phones while the PC audio element remains paused.
    if (['play', 'playTrack', 'next', 'previous', 'manualAd', 'seek'].includes(action) && updated?.playback?.status === 'playing') {
  ...
  const toggleMic = async () => {
    ...
    if (station.microphone?.enabled) {
      // Another operator may have started an announcement. An explicit stop from the desk
      // is allowed to take ownership back; a missing local ref must not make the button a
      // no-op while the room is still hearing the old announcement.
      try { await control('microphoneStop') } catch {}
      return
    }
    const m = createMicStreamer({ onDropped: message => { if (micRef.current === m) micRef.current = null; alert(message) } })
    // Reserve the button before awaiting permission/network. This closes the double-click
    // race that used to create two microphone sessions while the first getUserMedia dialog
    // was still open.
    micRef.current = m
    try { await m.start() }
    catch (err) { if (micRef.current === m) micRef.current = null; const msg = String(err?.message || err).slice(0, 140); alert('Mikrofona erişilemedi: ' + msg + '\n(Yönetim panelini bilgisayarda/Electron üzerinde açın; tarayıcıda mikrofon için 127.0.0.1 veya HTTPS gerekir.)') }
  }
...
function Listener() {
  const station = useStation(); const [playing, setPlaying] = useState(false); const audio = useRef(); const micRef = useRef(null); const tapRef = useRef(0); const tapTimer = useRef(); const actx = useRef(null); const gain = useRef(null); const adminPrompted = useRef(false)
  ...
  const [vol, setVol] = useState(() => readStoredVolume('cr_listen_vol'))
  const [admin, setAdmin] = useState(() => { try { return trustedAdminPage() && !!localStorage.getItem('cr_admin_token') } catch { return false } })
  ...
  useEffect(() => { applyVol(vol) }, [vol, Boolean(station)])
  ...
  useEffect(() => {
    ...
    if (!station || location.hash !== '#admin' || stored || adminPrompted.current) return
    adminPrompted.current = true
    if (!trustedAdminPage()) { openSecureAdmin(station); return }
  ...
  const toggleMic = async () => {
    ...
    if (station.microphone?.enabled) { try { await control('microphoneStop') } catch {} ; return }
    if (!window.isSecureContext) { openSecureAdmin(station); return }
...
export { Admin, BroadcastNotice, Dashboard, Listener, ConnectCard, UpdateCard, HistoryCard, UpNext, AdminCodeCard, EzanCard, Player, Progress, Status, TrackList, MasterBar, ErrorBoundary, IS_IOS, adminHeaders }
